...
namespace ItemAPI
{
    public static class ResourceExtractor
    {
        ...
        // alternative names as this technically checks prefixes, not folders:
        // GetEmbeddedTexturesFromPath
        // GetEmbeddedTexturesWithPathPrefix
        public static List<Texture2D> GetTexturesFromEmbeddedDirectory(string directoryPath)
        {
            ...
            foreach (var item in ResourceExtractor.GetResourceNames())
            {
                if (item.EndsWith(".png") && item.StartsWith(directoryPath))
                {
                    textures.Add(ResourceExtractor.GetTextureFromResource(item));
                }
            }
